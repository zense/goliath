...
// Si en cambio estoy en un nuevo post	
	...
	if (isset($err) and $err == "" ) {
		...
		if ($_GET['id']) {
			...
			if (!empty($_POST['external_url']))
				update_post_meta( $_POST['id'], '_external_url', $_POST['external_url']);
			$post_id = $_POST['id'];
		}else {
			$post_id = wp_insert_post($post);
			add_post_meta($post_id, 'wp-svbtle-markdown', $_POST['post_content']);
			if (!empty($_POST['external_url']))
				add_post_meta($post_id, '_external_url', $_POST['external_url']);
		}
	...
	if (isset($_GET['id']) and (get_post($_GET['id']))) {
		...
		if (get_post_meta($_GET['id'], '_external_url', true)) {
			$external_url = get_post_meta($_GET['id'], '_external_url', true);
		}else {
			$external_url 	= "";
		}
